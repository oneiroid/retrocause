...
Three times the true merges, none of them wrong. The two it can never
...
optional; it is not enough to wire into `sameInContext` unattended. The
next step is an adversarial set the author did not also design, and the
merge should stay opt-in behind a flag until then.
