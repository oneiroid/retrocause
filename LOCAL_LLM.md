...
**The one job this checkpoint is good at — bounded discrimination**
(measured 2026-08-17, `experiments/same_state.js`, on
`qwen3-1.7b-base-Q8_0` / reference profile — one model, so every claim
in this section is about the checkpoint until the cross-family check
below says otherwise). Everything it had
failed at was open-ended *generation*: invent a continuation, invent a
vocabulary, fill an array. Those failure modes — 37 variables for a
14-node story, repetition loops, confabulated facts — are all failures of
choosing from a large space, and say nothing about handing it two things
and asking for one bit.

`defaultContentKey` is surface form, which §5.5.4 names as the weak point
of the whole merge mechanism: states that mean the same thing in
different words never merge. That is unsolvable symbolically and is
exactly one boolean wide. On 18 hand-labelled pairs (9 same, 9
different):

| | model alone | + argument guard |
|---|---|---|
| accuracy | 15/18 | 15/18 |
| same pairs recognised | 7/9 | 6/9 |
| different pairs recognised | **8/9** | **9/9** |

Read the error *direction*, not the score. A missed merge costs nothing —
it is today's behaviour. A wrong merge rewires edges and deletes a node,
and is not recoverable. The model's one false positive was
`arrive(wolf, grandmother_house)` judged the same as
`arrive(red, grandmother_house)`: identical frame, swapped actor.

**So split the job where each side is exact.** Entity identity is
something code settles perfectly; synonymy is something code cannot touch.
The guard refuses any pair whose expr arguments differ, before the model
is consulted. That converts the destructive error into a conservative one
— same headline accuracy, and every remaining error now fails toward "do
not merge".

What it buys over today, on the same 18 pairs:

| | merges found | false merges |
|---|---|---|
| surface-form key (today) | 2/9 | 0 |
| model + argument guard | **6/9** | **0** |

Three times the true merges, none of them wrong. (One footnote from a
cross-model check, 2026-08-22: "the surface key makes no false merges" is
true only by rescue — at the content level it false-positives on the
recurrence pair, `cry(boy, wolf)` twice with different worlds, and the
incomparability clause is what saves it in-graph. The model key got that
pair right on content alone.) The two it can never
reach are blocked by the guard on purpose:
`arrive(red, grandmother_house)` / `reach(red, house)` and
`devour(wolf, flock)` / `kill(wolf, sheep)` — place and entity synonymy,
traded away deliberately because a destructive merge is unrecoverable and
these are rare inside one story's vocabulary.

**Caveats that keep this from being a benchmark.** Eighteen pairs, chosen
by the same author who labelled them, and clean ones. It is enough to say
the capability is real and to say the guard is mandatory rather than
...
```
FALSE MERGES: 2
  tell(red, wolf, grandmother_house) / tell(wolf, red, grandmother_house)
  swallow(wolf, grandmother)         / swallow(wolf, grandmother)
```

Both in the destructive direction, and each exposes a different hole:

1. *Role swap.* The guard SORTS arguments, so `tell(red, wolf, X)` and
   `tell(wolf, red, X)` reach the model, which judged "the wolf knows
   Red's destination" the same as "Red knows the wolf's destination".
   This one has a symbolic fix: compare arguments IN ORDER. Cost,
   computed from the recorded verdicts without a rerun: the symmetric
...
not as an automatic merge. The original write-up said "assume the
weakness until measured"; measured, and the weakness is real but
localised to aspect/tense inside identical frames.

**Cross-family check (2026-08-22): the hole replicates, the capability
does not.** Every number above was n=1 on models, so both probe sets
were rerun on a second base model of the same size and quant from a
different family and pretraining corpus: `smollm2-1.7b-base-Q8_0.gguf`
(converted from the official `HuggingFaceTB/SmolLM2-1.7B` by the same
`export_gguf.py` pipeline, sha256 `9e953cd7…5adcab`), served on the
same flags, same `same.v1` prompt, same seed. The Qwen numbers
reproduced exactly from live runs first — 15/18 / 0 false merges clean,
9/12 / 2 false merges adversarial — so this is an A/B, not a memory.

| guarded | Qwen3-1.7B-Base | SmolLM2-1.7B |
|---|---|---|
| clean: merges found | 6/9 | **2/9** |
| clean: false merges | 0 | 0 |
| adversarial: role-swaps caught | 1/2 | 2/2 |
| adversarial: false merges | 2 | **1** (aspect: `arrive` at-the-door vs at-the-bedside) |

Two findings, one per direction:

1. **The destructive failure mode is family-independent.** SmolLM2 got
   right both pairs Qwen false-merged — and then false-merged a
   *different* aspect pair inside an identical frame (`arrive(red,
   grandmother_house)` outside-the-door vs at-the-bedside). Two
   families, two different pairs, same category: aspect distinctions
   carried entirely by the state text, exactly the half of the split no
   guard can reach. The confirm-before-merge verdict is therefore not a
   checkpoint quirk; treat it as a property of the size class until a
   model clears the adversarial set.
2. **The useful capability is checkpoint-specific.** SmolLM2's merge
   recall is 2/9 — *exactly* the surface-form key's 2/9, so on that
   model the hybrid buys zero true merges over today's behaviour. "Small
   base models can do bounded discrimination" is not supported; "this
   Qwen3 checkpoint can" is. The model choice in §4 is load-bearing for
   the merge surface, not interchangeable within the size class.

The SmolLM2 artifact stays in the sibling workspace next to the
reference GGUF; nothing in this repo pins it, because nothing ships on
it — it exists as the other arm of this measurement.
...
1. *Accumulated state as an experimental `contentKey`* — **built
   ...
   **Measured: this checkpoint cannot do this extraction** (2026-08-17,
   `tools/extract_state.js`, red, 14 nodes, `qwen3-1.7b-base-Q8_0` on
   the reference profile — n=1 on models; the negative is recorded
   against the checkpoint, not the size class, and stays unreplicated
   because the design routed around it: `effects` are hand-authored
   ground truth regardless). It was
