...
| R8 | **Pin is a durable commitment, with explicit invalidation.** A `mergedState` is a *commitment* the merged node makes about its world-state (the §1.6 "declares its committed facts" reading), so it persists across later graph operations rather than silently recomputing. It is cleared at exactly the two points where the user means to override the commitment: (a) **explicit node edit** (`saveSelectedNodeEdits`) — the editor's `state`/`expr` fields then govern again; (b) **a new canonical parent** added to the node via the app `addEdge` wrapper — its predecessor set changed, so the pin no longer reflects a single agreed state. Other mutations leave the commitment intact by design (documented residual: removing a canonical parent does not auto-clear; re-edit the node to recompute). |
